...
public class BasicController {
    ...
    // - 생정자를 통한 의존성 주입, setter 메서드를 통한 의존성 주입, 필드를 통한 의존성 주입
    // - 생정자를 통한 의존성 주입을 spring 에서 권장 (의존성이 주입되지 않은 경우가 존재할 수 없기 때문)
    ...
    // HTTP GET Method : 클라이언트가 서버로부터 데이터를 받기를 원할 때 사용하는 메서드 ex) 회원가입
    ...
    // HTTP POST Method : 클라이언트가 서버에 데이터를 작성하기를 원할 때 사용하는 메서드 ex) 유저 정보 보기, 로그인(실제
    // 동작은 GET이나 보안 상의 문제로 POST 사용)
    ...
    // HTTP PUT Method : 클라이언트가 서버에 있는 리소스 전체를 수정하기를 원할 때 사용하는 메서드 ex) 게시물 수정하기(때에
    // 따라 PATCH로 쓸 때도 있음)
    ...
    // HTTP PATCH Method : 클라이언트가 서버에 있는 리소스 일부를 수정하기를 원할 때 사용하는 메서드
    ...
    // HTTP DELETE Method : 클라이언트가 서버에 있는 리소스를 삭제하기를 원할 때 사용하는 메서드 ex) 유저 정보 삭제
