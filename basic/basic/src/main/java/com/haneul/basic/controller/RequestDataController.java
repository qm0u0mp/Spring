...
public class RequestDataController {
    ...
    public String getRequestParam(
            ...
            @RequestParam(name = "userName", required = false) String userName, // required = false => 필수값이 아님
    ...
    @DeleteMapping({ "/path-variable/{age}/{name}", "/path-variable/{age}" })
    // http://localhost:4000/request-data/path-variable/10
    public String deletePathVariable(
            @PathVariable("age") Integer age, // int로 작성할 경우 0이 들어오게 되는데 사용자가 0을 입력한 것인지 기본값이 들어온 건지 알 수 없게 됨
    ...
    public String getPathVariable1(@PathVariable("value") String value) {
        return "getPathVariable1";
    }

    @GetMapping("/get/{value}")
    public String getPathVariable2(@PathVariable("value") String value) {
    ...
    // HTTP POST : http://localhost:4000/request-data/post
    @PostMapping("/post")
    public String post(
